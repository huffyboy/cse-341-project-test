...
export const announcementResolvers = {
    ...
    Mutation: {
        createAnnouncement: async (_: unknown, { input }: { input: AnnouncementInput }) => {
            const validatedData = validateAnnouncement(input);
            const announcement = new Announcement(validatedData);
            return await announcement.save();
        },
        updateAnnouncement: async (_: unknown, { id, input }: { id: string; input: AnnouncementUpdateInput }) => {
            const validatedData = validateAnnouncementUpdate(input);
            return await Announcement.findByIdAndUpdate(id, validatedData, { new: true });
        },
